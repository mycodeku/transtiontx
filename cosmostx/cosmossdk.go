...
func (self *CosmosSdk) CreateTx(ChainId string, CoinType string) client.TxBuilder {
	...
	sig := signing.SignatureV2{
		...
		Data: &signing.SingleSignatureData{
			SignMode: swichSigMode(self.SigMode),
		},
...
func (self *CosmosSdk) CreatTxByte(builder client.TxBuilder, ChainId string) (string, string) {
	initClientCtx := client.Context{}.WithChainID(ChainId)
	initClientCtx = initClientCtx.WithTxConfig(simapp.MakeTestEncodingConfig().TxConfig)
	...
	signBytes, err := initClientCtx.TxConfig.SignModeHandler().GetSignBytes(swichSigMode(self.SigMode), signerData, builder.GetTx())
	if err != nil {
		fmt.Println(err)
	}
...
func (self *CosmosSdk) GetTxWithSignature(sign string, builder client.TxBuilder, ChainId string) (string, string) {
...
func (self *CosmosSdk) PubKey() cryptotypes.PubKey {
